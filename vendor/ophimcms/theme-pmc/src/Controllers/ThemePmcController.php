...
class ThemePmcController
{
    public function index(Request $request)
    {
        if ($request['search'] || $request['filter']) {
            ...
            //đổi để ẩn hiện phim
            $data = Movie::where('is_published', true)
                ->when(!empty($request['filter']['category']), function ($movie) {
                    //
        ...
        return view('themes::themepmc.index', [
            ...
            return response()
            ->view('themes::themepmc.catalog', [
                'data'         => $data,
                'search'       => $request['search'],
                'section_name' => "Tìm kiếm phim: $request->search",
            ])
            // KHÔNG cache kết quả tìm kiếm / lọc
            ->header('Cache-Control', 'no-store, no-cache, must-revalidate');
        }

    // Không search / filter => trang home, cho cache 10 phút
    //--- Tắt Trang chủ ---
        //die();
        return response()
            ->view('themes::themepmc.index', [
                'title' => Setting::get('site_homepage_title'),
            ])
            // Cloudflare (Edge) sẽ cache theo header nàyz
            ->header('Cache-Control', 'public, max-age=300, s-maxage=300, must-revalidate, stale-while-revalidate=31536000');
    ...
    public function getMovieOverview(Request $request)
    {
        ...
        //ẩn hiện phim
        if (is_null($movie) || !$movie->is_published) {
            abort(404);
        }
        ...
        if(is_null($movie_related)) {
            ...
            ->where('is_published', true) // Loại trừ phim đã ẩn
        ...
        return response()-> view('themes::themepmc.single', [
            'currentMovie' => $movie,
            'title' => $movie->getTitle(),
            'movie_related' => $movie_related
        ])->header('Cache-Control', 'public, max-age=300, s-maxage=300, must-revalidate, stale-while-revalidate=31536000');
        //->header('Cache-Control', 'public, s-maxage=180, max-age=300, must-revalidate');
    }
    ...
    public function getEpisode(Request $request)
    {
       ...
       /*
        ...
        // thay return view sang return response()-> view để cache
        return response()-> view('themes::themepmc.episode', [
            'currentMovie' => $movie,
            'movie_related' => $movie_related,
            'episode' => $episode,
    ...
    public function updateMovieView(Request $request)
    {
        ...
        $updated = Movie::query()
            ...
            ->update([
                ...
                'view_month' => DB::raw('view_month + 1'),
    ...
    public function getMovieOfCategory(Request $request)
    {
        ...
        $movies = $category->movies()->where('is_published', true)->orderBy('updated_at', 'desc')->paginate(25); //ẩn phim nếu is_published true
    ...
    public function getMovieOfRegion(Request $request)
    {
        ...
        $movies = $region->movies()->where('is_published', true)->orderBy('updated_at', 'desc')->paginate(25); //ẩn phim nếu is_published true
    ...
    public function getMovieOfActor(Request $request)
    {
        ...
        $movies = $actor->movies()->where('is_published', true)->orderBy('updated_at', 'desc')->paginate(25);
    ...
    public function getMovieOfDirector(Request $request)
    {
        ...
        $movies = $director->movies()->where('is_published', true)->orderBy('updated_at', 'desc')->paginate(25);
    ...
    public function getMovieOfTag(Request $request)
    {
        ...
        $movies = $tag->movies()->where('is_published', true)->orderBy('created_at', 'desc')->paginate(25);
    ...
    public function getMovieOfType(Request $request)
    {
        ...
        if(is_null($movies)) {
            ...
            try {
                ...
                $movies = \Ophim\Core\Models\Movie::where('is_published', true)->when($relation_table, function ($query) use ($relation_table, $relation_field, $relation_val, $field, $val) {
        ...
        if ($request->ajax()) {
            ...
            switch ($key) {
                ...
                */
                ...
                case 'phim-2024':
                    $movies = Movie::where('is_shown_in_theater', 1)->where('publish_year', 2024)->orderBy('updated_at', 'desc')->take(12)->get();               
                break;
                
                case 'phim-2025':
                    $movies = Movie::where('is_shown_in_theater', 1)->where('publish_year', 2025)->orderBy('updated_at', 'desc')->take(12)->get();               
                break;
