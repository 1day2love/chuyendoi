...
class Episode extends Model implements Cacheable, HasUrlInterface, SeoInterface
{
    ...
    */
    ...
    public function generateSeoTags()
    {
        ...
        $movie_thumb_url = filter_var($this->movie->thumb_url, FILTER_VALIDATE_URL) ? $this->movie->thumb_url : 'https://' . $imageDomain . str_replace('/storage', '', parse_url($this->movie->thumb_url, PHP_URL_PATH));
        $movie_poster_url = filter_var($this->movie->poster_url, FILTER_VALIDATE_URL) ? $this->movie->poster_url : 'https://' . $imageDomain . str_replace('/storage', '', parse_url($this->movie->poster_url, PHP_URL_PATH));
