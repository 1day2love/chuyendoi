    ...
    */
    ...
    public function generateSeoTags()
    {
        ...
        if(count($this->movies)) {
            ...
            $movie_thumb_url = filter_var($this->movies->last()->thumb_url, FILTER_VALIDATE_URL) ? $this->movies->last()->thumb_url : 'https://' . $imageDomain . str_replace('/storage', '', parse_url($this->movies->last()->thumb_url, PHP_URL_PATH));
            $movie_poster_url = filter_var($this->movies->last()->poster_url, FILTER_VALIDATE_URL) ? $this->movies->last()->poster_url : 'https://' . $imageDomain . str_replace('/storage', '', parse_url( $this->movies->last()->poster_url, PHP_URL_PATH));
