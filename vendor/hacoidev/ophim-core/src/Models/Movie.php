...
class Movie extends Model implements TaxonomyInterface, Cacheable, SeoInterface
{
    ...
    */
    ...
    {
        $thumbUrl = str_replace('/storage', '', $this->thumb_url); // Loại bỏ '/storage'
        //$image_url = filter_var($thumbUrl, FILTER_VALIDATE_URL) ? $thumbUrl : str_replace('://', '://img.', url('/')) . $thumbUrl;
        //$image_url = filter_var($thumbUrl, FILTER_VALIDATE_URL) ? $thumbUrl : str_replace('://', '://img.', url('/')) . ltrim($thumbUrl, '/'); // Thêm img. xóa / trước
        $image_url = filter_var($thumbUrl, FILTER_VALIDATE_URL) ? $thumbUrl : preg_replace('/^(https?:\/\/)(?!img\.)/', '$1img.', url('/')) . $thumbUrl; // Thêm img. vào trước domain
    ...
    {
        $posterUrl = str_replace('/storage', '', $this->poster_url); // Loại bỏ '/storage'
        $image_url = filter_var($posterUrl, FILTER_VALIDATE_URL) ? $posterUrl : preg_replace('/^(https?:\/\/)(?!img\.)/', '$1img.', url('/')) . $posterUrl;
        if ($this->poster_url) {
            if (setting('site_image_proxy_enable', false)) {
                return str_replace('{image_url}', urlencode($image_url), setting('site_image_proxy_url', ''));
            }
    ...
    public function generateSeoTags()
    {
        ...
        $imageDomain = 'img.' . $currentDomain;
        $movie_thumb_url = filter_var($this->thumb_url, FILTER_VALIDATE_URL) ? $this->thumb_url : 'https://' . $imageDomain . str_replace('/storage', '', parse_url($this->thumb_url, PHP_URL_PATH));
        $movie_poster_url = filter_var($this->poster_url, FILTER_VALIDATE_URL) ? $this->poster_url : 'https://' . $imageDomain . str_replace('/storage', '', parse_url($this->poster_url, PHP_URL_PATH));
