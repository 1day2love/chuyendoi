...
<div id="header">
    <div class="container">
        <div id="logo" style="margin:0;">
            <a href="/" title="{{ $title }}">@if ($logo) {!! $logo !!} @else {!! $brand !!} @endif </a>
        </div>
        <ul id="main-menu">
    ...
    <li><a title="{{ $item['name'] }}" href="{{ $item['link'] }}" rel="nofollow">{{ $item['name'] }}</a>
        <ul class="sub-menu span-6">
          @foreach ($item['children'] as $children)<li><a class="sub-menu-link" href="{{ $children['link'] }}">{{ $children['name'] }}</a></li>@endforeach
        </ul></li>
    @else
    @if ($loop->last)
    <li class="vip"> <a title="{{ $item['name'] }}" href="{{ $item['link'] }}">{{ $item['name'] }}</a> </li>
    @else
    ...
    @endif @endforeach
        ...
        <form method="GET" id="form-search" class="form-search" action="/">
            <input placeholder="Tìm tên phim, thông tin..." value="{{ request('search') }}" type="text" name="search" />
            <i id="searchsubmit" class="fa fa-search" value="" type="submit" onclick="$('#form-search').submit();"></i>
        </form>
    </div>
</div>
<div id="mobile-header">
    <div class="btn-humber"></div>
    <a class="logo" href="/" title="Phim Mới">@if ($logo)  {!! $logo !!} @else {!! $brand !!}  @endif </a>
    ...
    <div class="mobile-search-bar hide">
        <form method="GET" id="mobile-form_search" class="mobile-form_search" action="/">
...
<div id="menu-mobile" class="">
    <ul> @foreach ($menu as $item) @if (count($item['children']))
    ...
    <ul class="sub-menu span-6" style="display: none;">
        @foreach ($item['children'] as $children)  <li><a class="sub-menu-link" href="{{ $children['link'] }}">{{ $children['name'] }}</a> </li> @endforeach</ul></li>
        @else
        @if ($loop->last)<li class="vip"> <a title="{{ $item['name'] }}" href="{{ $item['link'] }}">{{ $item['name'] }}</a></li> @else
         <li> <a title="{{ $item['name'] }}" href="{{ $item['link'] }}">{{ $item['name'] }}</a> </li>
        @endif
        @endif @endforeach
    </ul>
