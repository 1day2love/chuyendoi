...
@section('content')
    <div class="container" id="detail-page">
        ...
        <div class="film-info" itemscope itemtype="https://schema.org/Movie">
                    ...
                    <ul class="list-button">
                        @if ($currentMovie->trailer_url)
                            ...
                            @if ($currentMovie->status == 'trailer')
                                 <a class="btn btn-download btn-success" onclick="trailer();"
                                    title="Trailer {{ $currentMovie->name }}-{{ $currentMovie->origin_name }}">
                                    <i class="fa fa-youtube-play"></i> Trailer
                                </a>
                            @else
                                <a class="btn btn-download btn-info" onclick="trailer();"
                                title="Trailer {{ $currentMovie->name }}-{{ $currentMovie->origin_name }}">
                                <i class="fa fa-youtube-play"></i> Trailer
                                </a>
                                @endif  
                        ...
                        @endif
                        @if ($watch_url && $currentMovie->status !== 'trailer')
            ...
            <div class="clear"></div>
            @if(!isset($currentMovie->type) || !isset($currentMovie->status) || !in_array(strtolower($currentMovie->type), ['single']) || !in_array(strtolower($currentMovie->status), ['ongoing', 'completed'])) <!-- Phim lẻ không hiển thị -->
             <div class="latest-episode">
             @php
             if (strtolower($currentMovie->status) === 'trailer') {
            echo '<div class="block-film"><span class="heading"><strong>Phim hiện đang được cập nhật. Vui lòng quay lại sau!</strong></span></div>';
             } elseif (in_array(strtolower($currentMovie->status), ['ongoing', 'completed'])) {
            echo '<span class="heading">Tập mới nhất : </span>';
                     $currentMovie->episodes
                         ...
                         ->map(function ($episode) {
                          $episodeTitle = strpos($episode->name, 'Tập') !== false ? $episode->name : 'Tập ' . $episode->name;
                              echo '<a href="' . $episode->getUrl() . '">' . $episodeTitle . '</a>';
                         });
                     }
                 @endphp
            </div>
            ...
            <div class="text">
                ...
                <ul class="entry-meta block-film">
                    <li> <label>Đang phát: </label>
                        <span>
                        @if($currentMovie->status == 'trailer')
                            <font color="red">Sắp chiếu</font>
                        @elseif($currentMovie->type == 'single')
                            <font color="red">{{ $currentMovie->episode_current }} {{ $currentMovie->language }}</font>
                        @elseif($currentMovie->type == 'series')
                    ...
                    @endif
                    <li> <label>Năm Phát Hành: </label> {{ $currentMovie->publish_year ?? 'Đang cập nhật' }}
                    </li>
                    <li> <label>Quốc gia: </label>
                        {!! count($currentMovie->regions) 
                    ...
                    <li> <label>Diễn viên: </label>
                     @forelse($currentMovie->actors->take(10) as $actor)
                        <a href="{{ $actor->getUrl() }}" title="{{ $actor->name }}">{{ $actor->name }}</a>@unless($loop->last), @endunless
                     @empty
                            Đang cập nhật
                    @endforelse
                    </li>
            ...
            <div class="block-note">
                ...
                <div class="film-content block-film" id="film-content-wrapper">
                    ...
                    <div id="film-content">
                        @if ($currentMovie->content)
                          <!-- <strong> {{ $currentMovie->name }}</strong> -->{!! $currentMovie->content !!}
                        @else
                ...
                @if ($currentMovie->trailer_url)
                    <div class="block-film" style="display: none;" id="trailer">
                        ...
                        <script src="/build/js/jwplayer.js"></script>
                        <script>
             ...
             <!--
            <div class="comment">
                <div style="color:#000;padding: 0 10px;font-weight: bold;">Mong rằng bạn sẽ tiếp tục ủng hộ team bằng cách truy cập Phimtvi!</div>
...
@push('scripts')
    <script>
        ...
        var MOVIE_VIEW_URL = '{{ route('movie.update-view') }}';
    </script>

    <script defer type="text/javascript" src="{{ asset('/build/libs/jquery-raty/jquery.raty.js') }}"></script>
    <script defer type="text/javascript" src="{{ asset('/build/js/public.phim.js') }}"></script>
