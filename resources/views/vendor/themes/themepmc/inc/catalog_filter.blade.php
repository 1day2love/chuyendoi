        ...
        <ul>
            <li>
                <select class="form-control" id="category" name="filter[category]" form="form-search">
                    ...
                    @continue(!is_object($item) || !isset($item->id))
                            <option value="{{ $item->id }}" @if (request('filter.category') == $item->id ||
                                (isset($category) && is_object($category) && optional($category)->id == $item->id)) selected @endif>
                            {{ $item->name ?? '' }}</option>
                    @endforeach
                </select>
            </li>
            <li>
                <select class="form-control" name="filter[region]" form="form-search">
                    ...
                    @continue(!is_object($item) || !isset($item->id))
                    <option value="{{ $item->id }}" @if (request('filter.region') == $item->id ||
                                (isset($region) && is_object($region) && optional($region)->id == $item->id)) selected @endif>
                            Phim {{ $item->name }}</option>
                    @endforeach
                </select>
            </li>
            <li>
                <select class="form-control" name="filter[year]" form="form-search">
                    ...
                    @foreach ($years->slice(0, 40) as $year)
