...
@section('content')
    <div class="container" id="page-player">
        ...
        @endphp
            ...
            $mapServerToQuality = function ($serverName) {
            $server = Str::lower(Str::ascii((string) $serverName));
            $serverNoSpace = preg_replace('/[\s\-_]+/', '', $server);
            if (Str::contains($server, 'viet')) {
                return ['Vietsub', 'Vietsub'];
            }
            if (Str::contains($serverNoSpace, 'thuyetminh')) {
                return ['Thuyetminh', 'Thuyết Minh'];
            }
            if (Str::contains($serverNoSpace, 'longtieng')) {
                return ['Longtieng', 'Lồng Tiếng'];
            }
            $label = trim((string) $serverName);
        ...
        $qualityButtons = [];
        $currentQuality = null;
        foreach ($currentMovie->episodes->groupBy('server') as $serverName => $episodes) {
            [$qualityType, $qualityLabel] = $mapServerToQuality($serverName);
            if (!$qualityType) {
                continue;
            }
        $targetEpisode = $episodes->firstWhere('slug', $episode->slug);
            if (!$targetEpisode) {
        ...
        $buttonKey = md5($serverName . '|' . $url);
        if (isset($qualityButtons[$buttonKey])) {
            continue;
        }
        $sourceName = trim(Str::before($serverName, ' - '));
        $qualityButtons[$buttonKey] = [
            'type'   => $qualityType,
            'label'  => $qualityLabel,
            'source' => $sourceName,
            'server' => $serverName,
            'url'    => $url,
                ...
                if (
                (string) $episode->server === (string) $serverName &&
                (string) $episode->getUrl() === (string) $url
            ) {
                $currentQuality = $buttonKey;
            }
        }
        ...
        $qualityButtons = collect($qualityButtons)
            ->sortBy(function ($item) use ($qualityOrder) {
                return $qualityOrder[$item['type']] ?? 999;
            })
            ->toArray();
        @endphp

         @if (count($qualityButtons) > 1)
                    ...
                    @foreach ($qualityButtons as $key => $item)
                        @php $isActive = ($key === $currentQuality); @endphp
                            <a href="{{ $isActive ? 'javascript:;' : $item['url'] }}"
                                ...
                                data-type="{{ $item['type'] }}"data-server="{{ $item['server'] }}">
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    </div>
...
@push('scripts')
    ...
    <script src="https://ssl.p.jwpcdn.com/player/v/8.49.5/jwplayer.js"></script>
    ...
    <script>
       ...
       function chooseStreamingServer(el) {
         ...
         async function renderPlayer(type, link, id) {
            ...
            let urlNoAds = link;
            playkk = [ "phim1280","kkphimplayer1", "kkphimplayer2", "kkphimplayer3","kkphimplayer4", "kkphimplayer5", "kkphimplayer6", "kkphimplayer7", "kkphimplayer8"];
            const playop = [ "vipp" ];
            const shouldProcess = 
