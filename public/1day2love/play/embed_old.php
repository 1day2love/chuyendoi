...
checkRateLimit();
// Auto clear cache 30 ngày
autoClearCache30Days();
...
// Cache system đơn giản
function getCacheKey($url) {
    return 'm3u8_cache_' . md5($url);
}
...
function getFromCache($key) {
    ...
    if (file_exists($cacheFile)) {
        $data = json_decode(file_get_contents($cacheFile), true);
        if (time() - $data['timestamp'] < 300) { // Cache 5 phút
            return $data['content'];
        }
        unlink($cacheFile); // Xóa cache cũ
    }
    return null;
}
...
function fetchM3U8($url) {
    ...
    if (function_exists('curl_init')) {
        ...
        if ($httpCode === 200 && $content !== false && empty($error)) {
            // Lưu vào cache
...
function filterM3U8Segments($content, $baseUrl) {
    ...
    $content = removeConvertV7FromSegments($content);
...
//xóa cache 1 ngày 1 lần
function autoClearCache30Days() {
    if (mt_rand(1,50) !== 1) return;
    $flagFile = sys_get_temp_dir() . '/m3u8_cache_last_cleanup';
    $interval = 1 * 24 * 60 * 60; // 1 ngày
    ...
    $files = glob($cacheDir . '/m3u8_cache_*');
    ...
    if ($deleted > 0) {
        error_log("Auto cleared $deleted cache files (1 day)");
    }
}
